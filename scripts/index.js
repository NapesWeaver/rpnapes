...
function extractFirstValue(tmpString) {
  ...
  if (!/[=;,<>?:'"`~!@#$%√&×(){}[\]|\\_]/g.test(tmpString) && !/[Ω♥°a-df-hk-zA-DF-Z][-0-9]|[Ω♥°a-df-hk-zA-DF-Z][*/][0-9]/g.test(tmpString)) {
...
function btnUndo() {;
  ...
  if (shifted) {
    if (restores.length > 0) {
      redoFunction();
      if ($('lst-stack').clientHeight < 216 || $('lst-stack').scrollTop / $('lst-stack').scrollHeight > .63) $('lst-stack').scrollTop = $('lst-stack').scrollHeight;
    }
  } else {
    if (backups.length > 2)  {
      undoFunction();
      // If stackLength === 0 || cleintHeight is very small || ...
      if (stackLength === 0 || $('lst-stack').clientHeight < 216 || $('lst-stack').scrollTop / $('lst-stack').scrollHeight > .63) $('lst-stack').scrollTop = $('lst-stack').scrollHeight; 
    }
  }
  $('txt-input').select();
}
...
function help(command) {
  ...
  if (commandArray[1] !== undefined && radix !== 16) {
    ...
    case 'plot':
      inputText('');
      enterInput();
      inputText('plot: In RPN mode, the plot command graphs the last entry (expression, number or function) on the stack. Alternatively, the plot command may be followed by the argument e.g.\nplot 1∠45\nplot x^2\nFor AOS use plot() e.g.\nplot(function f(x) { return 1/x })\nplot(x=> sin(x))\nplot(\'1∠45\')');
      break;
    case 'polar':
      inputText('');
      enterInput();
